...
class CustomAgents(BaseAgents):
    ...
    def github_searcher(self):
        ...
        function_calling_llm
        """
        super().main_job(based_upon_agent="search")

        tempAgent = Agent(
            role=f"{self.role}find repo on github",
            backstory=dedent(f"""{self.backstory}you understand the github repository structure and can list all repositories for a certain profile or having certain keywords"""),
            goal=dedent(f"""{self.goal}find and list the repositories on github"""),
        ...
        markdown_file="./crews/GH-gh/agent_github_searcher.md"
        with open(markdown_file, 'w') as f:
            f.write("## github_searcher"+ '\n')
    ...
    def github_browser(self):
        """
        function_calling_llm
        """
        super().main_job(based_upon_agent="browse")

        tempAgent = Agent(
            role=f"{self.role}browse github repository  and summarize details",
            backstory=dedent(f"""{self.backstory}you are able to trace back repositories to the original repository source they are forked from"""),
            goal=dedent(f"""{self.goal}Figure out which forked repositories are behind on the original source repositories and need updating. This information is typically at the top of the page before the list of files"""),
            allow_delegation=True,
            verbose=True,
            tools=self.tools,
            llm=self.llm,
        ...
        # Function to write the extracted print statements into a markdown file.
        markdown_file="./crews/GH-gh/agent_github_browser.md"
        with open(markdown_file, 'w') as f:
            f.write("## github_browser"+ '\n')
            f.write(f"Role: {tempAgent.role}"+ '\n')
            f.write(f"Goal: {tempAgent.goal}"+ '\n')
            f.write(f"Backstory: {tempAgent.backstory}"+ '\n')
            f.write(f"Tools: {tempAgent.tools}"+ '\n')
