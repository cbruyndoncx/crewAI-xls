...
class CustomCrew:
    ...
    def run(self):
        ...
        # Define your custom agents and tasks here
        websearcher = agents.websearcher()

        
        # Custom tasks include agent name and variables as input
        search_for_major_players = tasks.search_for_major_players(
            websearcher,
            var1=self.job_to_do,
            var2=f"focus on content in {self.language}",
        )
        # Custom tasks include agent name and variables as input
        focus_areas = tasks.focus_areas(
            websearcher,
            var1=self.job_to_do,
            var2=f"focus on content in {self.language}",
        )
        # Custom tasks include agent name and variables as input
        key_strengths = tasks.key_strengths(
            websearcher,
            var1=self.job_to_do,
            var2=f"focus on content in {self.language}",
        )
        # Custom tasks include agent name and variables as input
        weaknesses = tasks.weaknesses(
            websearcher,
            var1=self.job_to_do,
            var2=f"focus on content in {self.language}",
        )
        ...
        groqresearch = Crew(
            agents=[webscraper,websearcher],
            tasks=[search_for_major_players,focus_areas,key_strengths,weaknesses],
            language="en",
        ...
        result = groqresearch.kickoff()

        metrics=groqresearch.usage_metrics
