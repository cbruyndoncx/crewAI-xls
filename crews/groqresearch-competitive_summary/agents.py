...
class CustomAgents(BaseAgents):
    ...
    def webscraper(self):
        ...
        markdown_file="./crews/groqresearch-competitive_summary/agent_webscraper.md"
    ...
    def websearcher(self):
        ...
        tempAgent = Agent(
            role=f"{self.role}internet news analyst",
            backstory=dedent(f"""{self.backstory}you are aware of the latest news sites and look for interesting noteworthy content related to the provided topics."""),
            goal=dedent(f"""{self.goal}find the latest news on specific topics"""),
            allow_delegation=True,
            verbose=True,
            tools=self.tools,
            llm=self.llm,
        ...
        # Function to write the extracted print statements into a markdown file.
        markdown_file="./crews/groqresearch-competitive_summary/agent_websearcher.md"
        with open(markdown_file, 'w') as f:
            f.write("## websearcher"+ '\n')
            f.write(f"Role: {tempAgent.role}"+ '\n')
            f.write(f"Goal: {tempAgent.goal}"+ '\n')
