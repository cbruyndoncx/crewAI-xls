...
class CustomAgents(BaseAgents):
    ...
    def translator(self):
        ...
        function_calling_llm
        """
        super().main_job(based_upon_agent="")

        tempAgent = Agent(
            role=f"{self.role}translate from english to dutch",
            backstory=dedent(f"""{self.backstory}you are an expert translator skilled in both english reading and dutch reading and writingg"""),
            goal=dedent(f"""{self.goal}generate final result in dutch"""),
        ...
        markdown_file="./crews/groqscrape-translate/agent_translator.md"
    ...
    def webscraper(self):
        ...
        # Function to write the extracted print statements into a markdown file.
        markdown_file="./crews/groqscrape-translate/agent_webscraper.md"
        with open(markdown_file, 'w') as f:
            f.write("## webscraper"+ '\n')
            f.write(f"Role: {tempAgent.role}"+ '\n')
            f.write(f"Goal: {tempAgent.goal}"+ '\n')
