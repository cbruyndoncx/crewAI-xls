...
class CustomAgents(BaseAgents):
    ...
    def youtube_searcher(self):
        ...
        function_calling_llm
        """
        super().main_job(based_upon_agent="websearch")

        tempAgent = Agent(
            role=f"{self.role}find info in youtube",
            backstory=dedent(f"""{self.backstory}you understand the youtube website structure and can list all videos for  certain keywords"""),
            goal=dedent(f"""{self.goal}find informaation on youtube"""),
            allow_delegation=True,
            verbose=True,
            tools=self.tools,
            llm=self.llm,
            max_iter=min(self.max_iter,10),
            max_rpm=min(self.max_rpm,60),
            step_callback="",
            memory=False,
        )

        # Function to write the extracted print statements into a markdown file.
        markdown_file="./crews/groqscrape-ytubers/agent_youtube_searcher.md"
        with open(markdown_file, 'w') as f:
            f.write("## youtube_searcher"+ '\n')
    ...
    def youtube_browser(self):
        """
        function_calling_llm
        """
        super().main_job(based_upon_agent="ytvideo")

        tempAgent = Agent(
            role=f"{self.role}get video details ",
            backstory=dedent(f"""{self.backstory}you are an expert in getting details on the video's published the user is interested in."""),
            goal=dedent(f"""{self.goal}get details on video content on youtube"""),
            allow_delegation=True,
            verbose=True,
            tools=self.tools,
            llm=self.llm,
        ...
        # Function to write the extracted print statements into a markdown file.
        markdown_file="./crews/groqscrape-ytubers/agent_youtube_browser.md"
        with open(markdown_file, 'w') as f:
            f.write("## youtube_browser"+ '\n')
            f.write(f"Role: {tempAgent.role}"+ '\n')
            f.write(f"Goal: {tempAgent.goal}"+ '\n')
            f.write(f"Backstory: {tempAgent.backstory}"+ '\n')
            f.write(f"Tools: {tempAgent.tools}"+ '\n')
    ...
    def webscraper(self):
        ...
        function_calling_llm
        """
        super().main_job(based_upon_agent="webscrape")

        tempAgent = Agent(
            role=f"{self.role}get website content from url",
            backstory=dedent(f"""{self.backstory}you are able to get content from a website url and extract the main body of the page"""),
            goal=dedent(f"""{self.goal}get current up to date information published on a website"""),
            allow_delegation=True,
            verbose=True,
            tools=self.tools,
            llm=self.llm,
            max_iter=min(self.max_iter,15),
            max_rpm=min(self.max_rpm,30),
            step_callback="",
            memory=False,
        )

        # Function to write the extracted print statements into a markdown file.
        markdown_file="./crews/groqscrape-ytubers/agent_webscraper.md"
        with open(markdown_file, 'w') as f:
            f.write("## webscraper"+ '\n')
            f.write(f"Role: {tempAgent.role}"+ '\n')
            f.write(f"Goal: {tempAgent.goal}"+ '\n')
