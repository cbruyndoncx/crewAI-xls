...
def read_variables_xls(template_filename, select_crew, select_job, crew_dir):
    ...
    # LLMs
    llm_list_template=''
    with open(f"{crew_dir}llm_providers.py", 'w') as file:
        ...
        for record in task_records:
            ...
            if record['job'] == select_job:
                ...
                if record['output_file'] : 
                    record['output_file'] = f"{crew_dir}output/{record['output_file']}"
    ...
    for record in crew_records:
        ...
        if record['crew_name'] == select_crew:
            ...
            with open(f"{crew_dir}crew.py", 'w') as file:
                file.write(result)
            return
